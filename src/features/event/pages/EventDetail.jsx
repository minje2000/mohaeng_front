...
const getStatusUI = (ev) => {
  if (!ev) return null;
  const today = new Date(); today.setHours(0,0,0,0);
  const d = (s) => { if (!s) return null; const dt = new Date(s); dt.setHours(0,0,0,0); return dt; };
          ...
          <div className="ed-card">
              ...
              <div className="ed-hero-right">
                ...
                <table className="ed-tbl">
                  <tbody>
                    ...
                    <tr>
                      <th>행사 장소</th>
                      <td>{[ev.detailAdr, ev.lotNumberAdr].filter(Boolean).join(' ') || '-'}</td>
                    </tr>
                    {ev.price != null && (
            ...
            {hashtags.length > 0 && (
              <div className="ed-tags">
                {hashtags.map((tag, i) => <span key={i} className="ed-tag">#{tag}</span>)}
              </div>
            )}
              ...
              <div className="ed-host-action">
                ...
                {statusUI && (
                  statusUI.btnActive && statusUI.btnTo ? (
                    <Link to={statusUI.btnTo} className="ed-main-btn active"
                      style={{background:statusUI.btnColor, color:statusUI.btnTextColor}}>
                      {statusUI.btnLabel}
                    </Link>
                  ) : (
            ...
            <div className="ed-tabs">
              {TABS.map(t => (
                <button key={t} className={`ed-tab-btn${tab === t ? ' active' : ''}`}
            ...
            {tab === '문의' && (
              <div className="ed-tab-content">
                <div className="ed-empty">
                  <div style={{fontSize:32, marginBottom:10}}>📩</div>
                  <div style={{fontWeight:700, color:'#9CA3AF'}}>문의 기능은 준비 중이에요.</div>
                </div>
              </div>
            )}
