...
import {
  ...
  getBoothApplicationInfo,
  getMyProfile,
} from '../api/ParticipationBoothApi';
...
const PLACEHOLDER =
  'https://dummyimage.com/400x400/f3f4f6/666666.png&text=Mohaeng';

const imgUrl = (path) => (path ? `${UPLOAD_BASE_EVENT}/${path}` : PLACEHOLDER);
const fileUrl = (path) => (path ? `${UPLOAD_BASE_HBOOTH}/${path}` : '#');
...
export default function ParticipationBoothApply() {
    ...
    const fetchData = async () => {
      try {
        ...
        const [eventRes, userRes] = await Promise.all([
          getBoothApplicationInfo(eventId),
          getMyProfile(),
        ]);
  ...
  const { boothPrice, facilityPrice, totalPrice } = useMemo(() => {
    ...
    const booth = eventData.booths.find(
      (x) => x.boothId === Number(selectedBoothId)
    );
    const bPrice = booth ? booth.boothPrice : 0;
    const fPrice = selectedFacilities.reduce((sum, sel) => {
      const facility = eventData.facilities.find(
        (f) => f.hostBoothfaciId === sel.hostBoothFaciId
      );
      return sum + (facility ? facility.faciPrice * sel.count : 0);
    }, 0);
    return {
      boothPrice: bPrice,
      facilityPrice: fPrice,
      totalPrice: bPrice + fPrice,
    };
    ...
    try {
      const dto = {
        ...
        facilities: selectedFacilities.map((f) => ({
          ...
          faciCount: f.count,
          faciPrice:
            (facilities.find((o) => o.hostBoothfaciId === f.hostBoothFaciId)
              ?.faciPrice || 0) * f.count,
        })),
      };
      await submitBoothApplication(eventId, dto, files);
      alert('부스 참가 신청이 제출되었습니다.');
  ...
  return (
    ...
    >
      ...
      >
        <h2
          style={{ fontSize: '26px', fontWeight: '900', marginBottom: '30px' }}
        >
          부스 참가 신청서
        </h2>
        ...
        <SectionBox title="행사 정보">
          ...
          >
            <img
              src={imgUrl(eventInfo.thumbnail)}
              alt="Thumbnail"
              style={{
                width: '150px',
                height: '150px',
            ...
            <div style={{ flex: 1 }}>
              ...
              >
                <span>
                  📅 <strong>일시:</strong> {fmtDate(eventInfo.startDate)} ~{' '}
                  {fmtDate(eventInfo.endDate)}
                </span>
                <span>
                  📍 <strong>장소:</strong>{' '}
                  {eventInfo.detailAdr || eventInfo.lotNumberAdr}
                </span>
              </div>
            </div>
          </div>
        ...
        >
            ...
            {booths.map((b) => (
              <label
                ...
                style={{
                  ...
                  border: '1.5px solid',
                  borderColor:
                    selectedBoothId === String(b.boothId)
                      ? THEME.primary
                      : THEME.border,
                  background:
                    selectedBoothId === String(b.boothId)
                      ...
                      : '#fff',
                  marginBottom: '10px',
                  cursor: b.remainCount === 0 ? 'not-allowed' : 'pointer',
                  opacity: b.remainCount === 0 ? 0.6 : 1,
                }}
              >
                <div
                  style={{
          ...
          <SectionBox title="부대시설 비용" disabled={!selectedBoothId}>
            {facilities.map((f) => {
              const sel = selectedFacilities.find(
                (x) => x.hostBoothFaciId === f.hostBoothfaciId
              );
              const isOut = f.hasCount && f.remainCount <= 0;

              return (
                <div
                  key={f.hostBoothfaciId}
                  style={{
                    display: 'flex',
                    justifyContent: 'space-between',
                    padding: '12px 0',
                      ...
                      onChange={(e) => {
                        if (e.target.checked)
                          setSelectedFacilities([
                            ...selectedFacilities,
                            { hostBoothFaciId: f.hostBoothfaciId, count: 1 },
                          ]);
                        else
                          setSelectedFacilities(
                  ...
                  >
                    ...
                    {sel && f.hasCount && (
                      <input
                        ...
                        onChange={(e) =>
                          setSelectedFacilities(
                            selectedFacilities.map((x) =>
                              x.hostBoothFaciId === f.hostBoothfaciId
                                ? {
                                    ...x,
                                    count: Math.min(
                                      f.remainCount,
        ...
        <div
          style={{
            ...
            alignItems: 'center',
            marginBottom: '30px',
            boxShadow: `0 10px 20px ${THEME.primary}44`,
          }}
        >
          <div>
            <span
            ...
            >
              부스 {boothPrice.toLocaleString()}원 + 시설{' '}
              {facilityPrice.toLocaleString()}원
            </div>
          </div>
          <span style={{ fontSize: '28px', fontWeight: '900', color: '#000' }}>
            {totalPrice.toLocaleString()}{' '}
            <small style={{ fontSize: '16px' }}>원</small>
          </span>
        </div>
        ...
        <SectionBox title="신청자(기업) 프로필">
          ...
          >
            로그인한 계정 정보로 자동 설정됩니다. 수정이 필요하면 마이페이지에서
            변경해주세요.
          </p>
          <div
            style={{
          ...
          >
            <div>
              <Label>이름</Label>
              <Input
                value={profile.name || '정보 없음'}
                readOnly
                disabled
                style={{
                  background: '#F3F4F6',
                  color: profile.name ? THEME.text : THEME.subText,
                }}
              />
            </div>
            <div>
              <Label>연락처</Label>
              <Input
                value={profile.phone || '정보 없음'}
                readOnly
                disabled
                style={{
                  background: '#F3F4F6',
                  color: profile.phone ? THEME.text : THEME.subText,
                }}
              />
            </div>
            <div style={{ gridColumn: '1/-1' }}>
              <Label>이메일</Label>
              <Input
                value={profile.email || '정보 없음'}
                readOnly
                disabled
                style={{
                  background: '#F3F4F6',
                  color: profile.email ? THEME.text : THEME.subText,
                }}
              />
            </div>
            <div style={{ gridColumn: '1/-1' }}>
        ...
        <SectionBox title="운영 계획">
          ...
          <div style={{ marginBottom: '20px' }}>
            ...
            <Input
              value={plan.items}
              onChange={(e) => setPlan({ ...plan, items: e.target.value })}
              placeholder="판매 물품 등"
            />
          </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '15px' }}>
          ...
          <button
            ...
            style={{
              ...
              borderRadius: '12px',
              border: 'none',
              background: isSubmitting ? THEME.border : THEME.primary,
              color: '#000',
              fontWeight: '900',
              fontSize: '16px',
              cursor: 'pointer',
