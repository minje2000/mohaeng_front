...
export default function ParticipationBoothApply() {
  ...
  const handleSubmit = async () => {
    ...
    } catch (e) { alert('제출 실패'); } finally { setIsSubmitting(false); }
  };
          ...
          <SectionBox title="부스 규격 및 비용">
            {booths.map(b => (
              <label key={b.boothId} style={{ 
                ...
                marginBottom: '10px', cursor: b.remainCount === 0 ? 'not-allowed' : 'pointer', opacity: b.remainCount === 0 ? 0.6 : 1
              }}>
                <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
                  {/* 💡 value={b.boothId} 추가됨 */}
                  <input type="radio" name="booth" value={b.boothId} disabled={b.remainCount === 0} checked={selectedBoothId === String(b.boothId)} 
                    onChange={(e) => setSelectedBoothId(e.target.value)} style={{ accentColor: THEME.secondary, marginTop: '18px' }} />
                  <div>
