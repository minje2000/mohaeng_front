...
const PBOOTH_BASE = 'http://localhost:8080/upload_files/pbooth';
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        fontSize: 14, fontWeight: 900, color: '#111827', background: '#F9FAFB',
      }}>
        {title}
      </div>
...
function Field({ label, children }) {
  return (
    <div style={{
      ...
      gap: 12, padding: '8px 0', borderBottom: '1px solid #F3F4F6',
    }}>
      <div style={{ fontSize: 12, fontWeight: 700, color: '#6B7280', paddingTop: 2 }}>
        {label}
      </div>
...
function StatusBadge({ status }) {
  const map = {
    PENDING:  { bg: '#FFF7ED', color: '#C2410C', border: '#FED7AA', label: '검토 중' },
    APPROVED: { bg: '#ECFDF5', color: '#047857', border: '#A7F3D0', label: '승인' },
    REJECTED: { bg: '#FEF2F2', color: '#B91C1C', border: '#FCA5A5', label: '반려' },
    CANCELED: { bg: '#F3F4F6', color: '#6B7280', border: '#E5E7EB', label: '취소' },
    ...
    결제완료: { bg: '#F0FDF4', color: '#15803D', border: '#BBF7D0', label: '결제완료' },
  };
  const s = map[status] || { bg: '#F3F4F6', color: '#374151', border: '#E5E7EB', label: status || '-' };
  return (
    <span style={{
...
export default function BoothApplicationDetailMypage() {
  ...
  if (loading) return <div style={{ padding: 20, color: '#6B7280', fontWeight: 800 }}>불러오는 중...</div>;
  if (error)   return <div style={{ padding: 20, color: '#991B1B', fontWeight: 900 }}>신청서를 불러오지 못했어요.</div>;
  if (!data)   return null;
    ...
    <div style={{
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '28px 32px 60px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
          ...
          <div style={{ fontSize: 22, fontWeight: 900, letterSpacing: '-0.03em', color: '#111827' }}>
            부스 신청서
          </div>
        </div>
        ...
        >
          ...
          <div style={{ minWidth: 0 }}>
            ...
            <div style={{ marginTop: 4, fontSize: 12, color: '#6B7280' }}>행사 상세로 이동 →</div>
          </div>
        </button>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
          <Section title="👤 신청자 정보">
            {data.applicantProfileImg && (
              <div style={{
                ...
                <img
                  src={`${PHOTO_BASE}/${data.applicantProfileImg}`}
                  alt="프로필"
          ...
          <Section title="📌 신청 상태">
            <Field label="상태"><StatusBadge status={data.status} /></Field>
            <Field label="신청일">{fmtDate(data.createdAt)}</Field>
            <Field label="승인일">{data.approvedDate ? fmtDate(data.approvedDate) : '-'}</Field>
          </Section>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
          <Section title="🏗️ 선택 부스">
            ...
            <Field label="단가">
              <span style={{ color: '#D97706', fontWeight: 900 }}>{fmt(data.boothUnitPrice)}원</span>
            </Field>
            <Field label="총 수량 / 잔여">
          ...
          <Section title="🔧 선택 부대시설">
            {facilities.length === 0 ? (
              <span style={{ fontSize: 13, color: '#9CA3AF' }}>선택한 부대시설 없음</span>
            ) : (
                  ...
                  <tr style={{ background: '#F9FAFB', borderBottom: '2px solid #E5E7EB' }}>
                    {['시설명', '단위', '수량', '단가', '금액'].map((h) => (
                      <th key={h} style={{
                        ...
                        fontWeight: 800, color: '#374151', whiteSpace: 'nowrap',
                      }}>
                        {h}
                      </th>
                    ))}
                ...
                <tbody>
                  {facilities.map((f, i) => (
                    <tr key={i} style={{ borderBottom: '1px solid #F3F4F6' }}>
                      <td style={{ padding: '10px 10px', fontWeight: 700 }}>{f.faciName || '-'}</td>
                      <td style={{ padding: '10px 10px', color: '#6B7280' }}>{f.faciUnit || '-'}</td>
                      <td style={{ padding: '10px 10px' }}>{f.faciCount ?? '-'}</td>
                      <td style={{ padding: '10px 10px', color: '#6B7280' }}>{fmt(f.unitPrice)}원</td>
                      <td style={{ padding: '10px 10px', fontWeight: 800, color: '#D97706' }}>
                        {fmt(f.faciPrice)}원
                      </td>
                    </tr>
                  ))}
        ...
        <Section title="💳 결제 금액">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16 }}>
            <div style={{ background: '#F9FAFB', borderRadius: 12, padding: '16px 20px', border: '1px solid #E5E7EB' }}>
              <div style={{ fontSize: 12, color: '#6B7280', fontWeight: 700, marginBottom: 6 }}>부스 비용</div>
              <div style={{ fontSize: 18, fontWeight: 900 }}>{fmt(boothPrice)}원</div>
            </div>
            <div style={{ background: '#F9FAFB', borderRadius: 12, padding: '16px 20px', border: '1px solid #E5E7EB' }}>
              <div style={{ fontSize: 12, color: '#6B7280', fontWeight: 700, marginBottom: 6 }}>부대시설 비용</div>
              <div style={{ fontSize: 18, fontWeight: 900 }}>{fmt(facilityPrice)}원</div>
            </div>
            <div style={{ background: '#111827', borderRadius: 12, padding: '16px 20px' }}>
              <div style={{ fontSize: 12, color: '#9CA3AF', fontWeight: 700, marginBottom: 6 }}>총 결제 금액</div>
              <div style={{ fontSize: 22, fontWeight: 900, color: '#fff' }}>{fmt(totalPrice)}원</div>
            </div>
          </div>
        </Section>
        ...
        <Section title="📝 부스 운영 계획">
          ...
          <Field label="상세 설명">
            {data.description
              ...
              : <span style={{ color: '#9CA3AF', fontWeight: 500 }}>상세설명 없음</span>
            }
          </Field>
        </Section>
        ...
        <Section title="📎 첨부파일">
          {files.length === 0 ? (
            <span style={{ fontSize: 13, color: '#9CA3AF' }}>첨부파일 없음</span>
          ) : (
            <>
              {files.filter((f) => isImg(f.renameFileName)).length > 0 && (
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: 12 }}>
                  {files.filter((f) => isImg(f.renameFileName)).map((f) => (
                    <a key={f.renameFileName} href={`${PBOOTH_BASE}/${f.renameFileName}`} target="_blank" rel="noreferrer">
                      <img
                ...
                {files.filter((f) => !isImg(f.renameFileName)).map((f) => (
                  <a
                    ...
                    style={{
                      ...
                      color: '#111827', fontWeight: 700, fontSize: 13, textDecoration: 'none',
                    }}
                  >
                    <span>📄</span>
                    <span>{f.originalFileName || f.renameFileName}</span>
                    <span style={{ marginLeft: 'auto', fontSize: 11, color: '#9CA3AF' }}>다운로드</span>
                  </a>
                ))}
              </div>
            </>
