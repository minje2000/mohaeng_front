...
const HASHTAGS = [
  {id:1,name:'즐거운'},{id:2,name:'평온한'},{id:3,name:'열정적인'},{id:4,name:'디지털디톡스'},
  {id:5,name:'창의적인'},{id:6,name:'영감을주는'},{id:7,name:'활기찬'},{id:8,name:'편안한'},
  {id:9,name:'트렌디한'},{id:10,name:'전문적인'},{id:11,name:'교육적인'},{id:12,name:'감성적인'},
  {id:13,name:'도전적인'},{id:14,name:'따뜻한'},{id:15,name:'유익한'},{id:16,name:'색다른'},
  {id:17,name:'미니멀한'},{id:18,name:'역동적인'},{id:19,name:'신선한'},{id:20,name:'친근한'},
  {id:21,name:'화려한'},{id:22,name:'조용한'},{id:23,name:'성장하는'},{id:24,name:'함께하는'},
  {id:25,name:'지속가능한'},{id:26,name:'흥미진진한'},{id:27,name:'진지한'},{id:28,name:'자유로운'},
  {id:29,name:'집중하는'},{id:30,name:'친환경적인'},
];
...
const inputBase = {
  ...
  outline:'none', transition:'border 0.15s', fontFamily:'inherit', background:'#fff',
};
        ...
        return (
          <label key={t.id} style={{display:'flex', alignItems:'center', gap:6, fontSize:13, fontWeight:700, color: checked ? '#111' : '#6B7280', cursor:'pointer', userSelect:'none'}}>
            <input type="checkbox" checked={checked} onChange={() => onToggle(t.id)}
              style={{accentColor:'#FFD700', width:15, height:15, cursor:'pointer', flexShrink:0}} />
            {t.name}
          </label>
        );
      })}
...
export default function EventHost() {
  ...
  const [form, setForm] = useState({
    ...
    zipCode:'', roadAddress:'', detailAdr:'', lotNumberAdr:'', detailAddressExtra:'',
    isFree: true, price: '', noLimit: false, capacity: '',
    startRecruit:'', endRecruit:'', boothStartRecruit:'', boothEndRecruit:'',
  });
  ...
  const validate = () => {
    ...
    if (hasBooth) {
      if (form.boothEndRecruit && form.endRecruit && form.boothEndRecruit > form.endRecruit) {
        alert(`부스 모집 종료일(${form.boothEndRecruit})은 행사 참여 모집 종료일(${form.endRecruit}) 이전이어야 해요.`);
        return false;
      }
        ...
        <SectionCard step="1" title="행사 기본 정보" icon="📋">
              ...
              <G2>
                <div>
                  ...
                  <Input type="date" value={form.startDate} min={today}
                    onChange={e => { setF('startDate', e.target.value); if (form.endDate && e.target.value > form.endDate) setF('endDate', ''); }} />
                </div>
                <div>
              ...
              <G2>
                <div>
                  ...
                  <Input type="time" value={form.startTime}
                    onChange={e => { setF('startTime', e.target.value); if (form.startDate === form.endDate && form.endTime && e.target.value > form.endTime) setF('endTime', ''); }} />
                </div>
                <div>
            ...
            <G2>
              <div>
                ...
                <Input type="date" value={form.startRecruit} min={today} max={maxEndRecruit || undefined}
                  onChange={e => { setF('startRecruit', e.target.value); if (form.endRecruit && e.target.value > form.endRecruit) setF('endRecruit', ''); }} />
              </div>
              <div>
          ...
          <div>
            ...
            <div style={{display:'flex', gap:8, marginBottom:8}}>
              ...
              <button onClick={openAddressSearch} style={{padding:'10px 18px', borderRadius:10, border:'none', whiteSpace:'nowrap', background:'#FFD700', color:'#111', fontWeight:900, fontSize:13, cursor:'pointer', flexShrink:0}}>🔍 주소 찾기</button>
            </div>
            {form.zipCode && (
...
/>
        ...
        <SectionCard step="3" title="주최자 정보" icon="🏢">
          <div style={{padding:'14px 16px', background:'#F9FAFB', borderRadius:12, border:'1px solid #E5E7EB', marginBottom:4}}>
            ...
            <G2>
              <div><Label>주최자/기업 명</Label><Input value={hostInfo.name || '(이름 정보 없음)'} readOnly style={{background:'#F3F4F6', color:'#6B7280', cursor:'not-allowed'}} /></div>
        ...
        <SectionCard step="4" title="상세 설명 / 준비물 / 유의사항" icon="📝">
          <div style={{marginBottom:16}}>
            ...
            <Textarea rows={6} placeholder="행사 상세 설명, 준비물, 유의사항 등을 자유롭게 작성해주세요."
        ...
        <SectionCard step="5" title="부스 & 부대시설" icon="🏪">
          ...
          {hasBooth && (<>
            <div style={{padding:'14px 16px', background:'#FFFBEB', borderRadius:12, border:'1px solid #FDE68A', marginBottom:16}}>
              ...
              {form.endRecruit && (
                <div style={{fontSize:11, color:'#B45309', marginBottom:10}}>
                  부스 모집 기간은 행사 참여 모집 종료일({form.endRecruit}) 이전이어야 해요.
                </div>
              )}
              <G2>
                <div>
                  <Label>부스 모집 시작일</Label>
                  <Input type="date" value={form.boothStartRecruit} max={form.endRecruit || undefined}
                ...
                <div>
                  ...
                  <Input type="date" value={form.boothEndRecruit} min={minBoothEndRecruit} max={form.endRecruit || undefined}
        ...
        <div style={{display:'flex', justifyContent:'flex-end', gap:12, paddingTop:8}}>
          ...
          <button onClick={handleSubmit} disabled={saving} style={{
            padding:'12px 40px', borderRadius:13,
            background: saving ? '#E5E7EB' : 'linear-gradient(135deg,#FFD700,#FFC200)',
            border:'none', color: saving ? '#9CA3AF' : '#111',
